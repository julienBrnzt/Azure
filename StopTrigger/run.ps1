...
Try{
    ...
    If($vmStatus1.statuses[-1].displayStatus -ne 'VM deallocated'){
        Stop-AzVM -ResourceGroupName $ResourceGroupName -Name $VMName1 -Force -Verbose
        [string]$message += "... Virtual machine [$VMName1] is stopping"
    }
    Else{ 
        [string]$message += "... Virtual machine [$VMName1] is already deallocated"
    }
    ...
    If($vmStatus2.statuses[-1].displayStatus -ne 'VM deallocated'){
        Stop-AzVM -ResourceGroupName $ResourceGroupName -Name $VMName2 -Force -Verbose
        [string]$message += "... Virtual machine [$VMName2] is stopping"
    }
    Else{ 
        [string]$message += "... Virtual machine [$VMName2] is already deallocated"
    }
